...
import com.myst25.quicksearch.search.contacts.utils.ContactIntentHelpers
...
import com.myst25.quicksearch.search.core.DirectDialChoice
import com.myst25.quicksearch.search.core.DirectDialOption
import com.myst25.quicksearch.search.core.MessagingApp
import com.myst25.quicksearch.search.core.PendingThirdPartyCall
import com.myst25.quicksearch.search.core.PhoneNumberSelection
...
class ContactActionHandler(
    ...
    private fun performCalling(
        ...
        when (
            ...
            CallingApp.WHATSAPP -> {
                val method =
                    contactInfo.contactMethods.firstOrNull { it is ContactMethod.WhatsAppCall && it.dataId != null && (it.data.isBlank() || com.myst25.quicksearch.search.utils.PhoneNumberUtils.isSameNumber(it.data, number)) } as? ContactMethod.WhatsAppCall
                if (method?.dataId != null) {
                    handleWhatsAppCallWithPermission(method.dataId)
                } else {
            ...
            CallingApp.SIGNAL -> {
                val method =
                    contactInfo.contactMethods.firstOrNull { it is ContactMethod.SignalCall && it.dataId != null && (it.data.isBlank() || com.myst25.quicksearch.search.utils.PhoneNumberUtils.isSameNumber(it.data, number)) } as? ContactMethod.SignalCall
                if (method?.dataId != null) {
                    handleSignalCallWithPermission(method.dataId)
                } else {
                    beginRegularCallFlow(contactInfo.displayName, number)
            ...
            CallingApp.GOOGLE_MEET -> {
                val method =
                    contactInfo.contactMethods.firstOrNull { it is ContactMethod.GoogleMeet && it.dataId != null && (it.data.isBlank() || com.myst25.quicksearch.search.utils.PhoneNumberUtils.isSameNumber(it.data, number)) } as? ContactMethod.GoogleMeet
                if (method?.dataId != null) {
